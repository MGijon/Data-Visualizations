...
def selectFlats():
    ...
    #prices_val = prices_slider.value
    #reviews_val = reviews_slider.value
    #avaiability_val = avaiability_slider.value

    data["price"] = pd.to_numeric(data["price"])
    
    selected = data[
        (data["price"] >= float(prices_slider.value)) & #prices_val) &
        (data["reviews_per_month"] >= float(reviews_slider.value)) #&
      #  (data["avaiability_365"] >= avaiability_slider)
    ]
